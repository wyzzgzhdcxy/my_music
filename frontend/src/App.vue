...
import { GetMediaFiles, GetLyric, SelectMusicFolder, ClearThumbnails, GetCover, LogMessage, OpenCacheFolder, GetWhttpAddr } from '../wailsjs/go/main/App'
import { WindowMinimise, Quit, WindowSetSize, WindowSetPosition, WindowGetPosition, WindowGetSize, WindowSetMinSize, WindowSetMaxSize, WindowSetAlwaysOnTop, WindowIsMinimised, WindowToggleMaximise, WindowIsMaximised, WindowFullscreen, WindowUnfullscreen, WindowIsFullscreen } from '../wailsjs/runtime/runtime'
...
// whttp server address for file serving
const whttpAddr = ref('')
...
const videoThumbnailUrl = computed(() => {
  ...
  if (!track?.thumbnailPath || !whttpAddr.value) return ''
  return `${whttpAddr.value}/file?path=${encodeURIComponent(track.thumbnailPath)}`
})
...
function onWindowRestore() {
  if (isMinimized.value) {
    ...
    if (wasVideoPlaying.value && isVideoMode.value) {
      nextTick(() => {
        videoPlayerRef.value?.play()
      })
      wasVideoPlaying.value = false
    }
  }
}
...
function startMinimizePolling() {
  ...
  minimizeCheckInterval = setInterval(async () => {
    ...
    if (isMin && !isMinimized.value) {
      isMinimized.value = true
      wasVideoPlaying.value = isVideoMode.value && audioPlayer.isPlaying.value
      if (audioPlayer.isPlaying.value) {
        visualizer.stopVisualizer()
      }
...
async function saveWindowState() {
  ...
  windowStateSaveTimer = setTimeout(async () => {
    try {
      const pos = await WindowGetPosition()
      const size = await WindowGetSize()
      // Only save if window is in a valid position (not off-screen)
      if (pos.x >= 0 && pos.y >= 0) {
...
async function restoreWindowState() {
  try {
    ...
    if (state && state.x !== undefined && state.y !== undefined && state.width && state.height) {
      // Validate position is on-screen (positive coordinates)
      // If saved position is off-screen, skip restore to prevent window auto-minimize
...
function loadTrack(index: number) {
  ...
  if (track.isVideo) {
    ...
    // 视频使用 /file 接口（/audio 只支持音频格式）
    const videoSrc = `${whttpAddr.value}/file?path=${encodeURIComponent(track.path)}`
    // 计算视频封面缩略图URL
    const thumbUrl = track.thumbnailPath
      ? `${whttpAddr.value}/file?path=${encodeURIComponent(track.thumbnailPath)}`
      : ''
    nextTick(() => {
      videoPlayerRef.value?.loadSrc(videoSrc, thumbUrl)
    })
  } else {
  ...
  if (track.backgroundPath) {
    backgroundUrl.value = `${whttpAddr.value}/file?path=${encodeURIComponent(track.backgroundPath)}`
  } else {
    backgroundUrl.value = ''
  }

  if (track.coverPath) {
    coverUrl.value = `${whttpAddr.value}/file?path=${encodeURIComponent(track.coverPath)}`
  } else {
    // 从网易云获取封面
    GetCover(track.artist || '', track.title || '').then(coverPath => {
      if (coverPath) {
        coverUrl.value = `${whttpAddr.value}/file?path=${encodeURIComponent(coverPath)}`
      } else {
        coverUrl.value = ''
      }
    })
...
async function loadPlaylist() {
  ...
  // 使用固定 URL
  const url = 'http://localhost:19891/file'
...
async function selectMusicFolder() {
  try {
    ...
    // 使用固定 URL
    audioPlayer.setStreamUrl('http://localhost:19891/file')
...
// ============== Watch minimize state - restore video on unminimize ==============
watch(isMinimized, (newVal, oldVal) => {
  if (oldVal === true && newVal === false && isVideoMode.value) {
    // Coming out of minimized mode while in video mode - reload video
    nextTick(() => {
      const idx = playlistState.currentIndex.value
      if (idx >= 0) {
        loadTrack(idx)
      }
    })
...
onMounted(async () => {
  ...
  stepStart = Date.now()
  whttpAddr.value = await GetWhttpAddr()
  LogMessage(`[前端]   - whttpAddr: ${whttpAddr.value}`)
  LogMessage(`[前端]   - GetWhttpAddr 耗时: ${Date.now() - stepStart}ms`)
  stepStart = Date.now()
      ...
      <template v-else>
        ...
        <div class="titlebar-controls" style="--wails-draggable: no-drag;">
          ...
          <MenuPanel
            ...
            @selectBgImage="theme.saveBgImage(); showMenu = false"
